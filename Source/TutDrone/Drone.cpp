...
ADrone::ADrone()
{
	...
	UpThruster->SetupAttachment(Mesh);
	UpThruster->ThrustStrength = OutCollision->GetMass() * 980;
	...
	ForwardThruster->SetupAttachment(Mesh);
...
void ADrone::Tick(float DeltaTime)
{
	...
	/*if (GetInputAxisValue(TEXT("Litf")) == 0.0f)
	{
	...
	if (GetInputAxisValue(TEXT("Forward")) == 0.0f)
	{
		...
		if (Mesh->GetRelativeRotation().Pitch!=0.0f)
		{
			...
			{
				Mesh->SetWorldRotation(FRotator(0.0f, 0.0f, 0.0f));
			}
		}
	}
...
void ADrone::Forward(float value)
{
	...
	{
		Mesh->AddRelativeRotation(FRotator(-5.0f * GetWorld()->DeltaTimeSeconds*value, 0.0f, 0.0f));
	}
